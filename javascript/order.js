document.addEventListener('DOMContentLoaded', () => {
    ...
    forms.forEach(form => {
        form.addEventListener('submit', (event) => {
            ...
            checkboxes.forEach(checkbox => {//loop through checkbox inputs
                const quantityInput = form.querySelector(`input[name="${checkbox.value}-qty"]`);
                const quantity = quantityInput.value;
                if (!quantity || quantity <= 0) {//validate empty fields
                    valid = false;
                    quantityInput.focus();
    ...
    function createProductRow(checkbox, formData) {//create product row to the cart
        const product = checkbox.id;
        const priceText = checkbox.nextElementSibling.innerText;
        const price = parseFloat(priceText.match(/\d+\.?\d*/)[0]);//get only the digits from pricetext
        const quantity = formData.get(`${product}-qty`);
        const total = (price * quantity).toFixed(2);

        const row = document.createElement('tr');//to make the 1st product letter capital in the cart
        row.innerHTML = `
        ...
        row.querySelector('.remove-btn').addEventListener('click', () => {//remove button function
            row.remove();
            updateCartTotal();
        });
    ...
    function updateCartTotal() {
        ...
        rows.forEach(row => {
            const rowTotal = parseFloat(row.cells[3].innerText);//acces the total column
            total += rowTotal;
        });
    ...
    function saveCartToFavorites() {
        ...
        localStorage.setItem('favorites', JSON.stringify(cartItems));//make the object as a string to store in local storage
    ...
    function applyFavoritesToCart() {
        ...
        if (favorites) {//check for any exixting favorites
...
function validateCheckoutForm() {
    ...
    if (email.value.trim() === "") {
        ...
        var emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;//Defines a regular expression to match a standard email format.
    ...
    if (phone.value.trim() === "") {
        ...
        var phoneRegex = /^\d{3}-\d{3}-\d{4}$/;//Defines a regular expression for phone numbers in the format 123-456-7890.
    ...
    if (zip.value.trim() === "") {
        ...
        var zipRegex = /^\d{5}$/;// validating to be 5 digits only
    ...
    if (cardNumber.value.trim() === "") {
        ...
        var cardNumberRegex = /^\d{16}$/;// validating to be 16 digits
    ...
    if (expiry.value.trim() === "") {
        ...
        var expiryDate = new Date(expiry.value + "-01");// validating expire date not to be a past date from the current date

        if (expiryDate < currentDate) {
            alert("Your card is expired. Please use a valid card.");
            expiry.classList.add("error");
            expiryError.style.display = "inline";
            isValid = false;
        }
    ...
    if (cvv.value.trim() === "") {
        ...
        var cvvRegex = /^\d{3}$/;// validate to be 3 digits
